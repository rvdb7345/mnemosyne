...
def normalize_text(text):
    """
    Normalize text by removing accents, handling Turkish 'I's, and converting to lowercase.

    Specifically, this function:
    - Maps Turkish 'I', 'İ', and 'ı' to 'i' to ensure consistent lowercase 'i' usage.
    - Removes diacritical marks (accents) from characters.
    - Converts the entire string to lowercase.
    """
    # Define a translation table for Turkish-specific characters
    turkish_mapping = str.maketrans({
        'I': 'i',   # Uppercase I (without dot) to lowercase i
        'İ': 'i',   # Uppercase İ (with dot) to lowercase i
        'ı': 'i',   # Lowercase ı (without dot) to lowercase i
    })

    # Apply the translation to handle Turkish 'I's
    text = text.translate(turkish_mapping)

    # Normalize the text to NFD to separate base characters and diacritics
    text = unicodedata.normalize('NFD', text)

    # Remove all combining characters (diacritics)
...
def compare_strings(a, b, tolerance, ignore_accents):
    ...
    return ratio * 100 >= tolerance, original_b, exact_match
...
def get_users(exercises_dir):
    ...
    users = [f for f in os.listdir(exercises_dir) if os.path.isdir(
        os.path.join(exercises_dir, f))]
    return users
...
def save_credentials(credentials_file, credentials):
    ...
    df = pd.DataFrame(list(credentials.items()),
    ...
    if uploaded_file is not None and custom_name:
        try:
            ...
            if uploaded_file.name.endswith('.txt'):
                ...
                df = pd.read_csv(cleaned_content, sep='\t', header=None, names=[
                                 source_language_name, target_language_name])

            elif uploaded_file.name.endswith('.csv'):
                df = pd.read_csv(uploaded_file, header=None, names=[
                                 source_language_name, target_language_name]).applymap(str.strip)
            else:
            ...
            st.success(
                f"Exercise '{custom_name}' uploaded successfully with languages {source_language_name} to {target_language_name}!")
...
def get_exercises(user, exercises_dir):
    ...
    files = [f for f in os.listdir(user_exercises_dir) if f.endswith(
        '.txt') or f.endswith('.csv')]
    return files
